...
import {Person, RolePosition} from '../../models'
...
export class RolePositionComponent implements OnInit {
  ...
  ngOnInit(): void {
      this.dataService.getRolePosition().subscribe(rolePosition =>
      [this.myData = rolePosition]
    );
  ...
  deleteItem(rolePosition: RolePosition){
  ...
  addItem(rolePosition: RolePosition) {
    this.dataService.addRolePosition(rolePosition).subscribe( (tt)=> {
      this.myData.push( tt );
      this.toggleForm();
    }
    );
  ...
  openDeleteModal(data:any) {
    ...
    dialogConfig.data = {
      ...
      name: "delRolePosition",
      title: `Hi ${userId}, está por eliminar una de las posiciones`,
